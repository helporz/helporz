...
- (void)pluginInitialize {
    ...
    self.pendingMessageMap = [NSMutableDictionary new];
...
-(void)didSendSingleTextMessage:(NSNotification *)notification {
    NSLog(@"JMessagePlugin didSendSingleTextMessage  %@",[notification.object toJsonString]);
    
    NSString *jsonString = [notification.object toJsonString];
//    NSLog(@"JMessage didReceiveJMessageMessage  %@",jsonString);
    ...
    NSString * msgId = [json objectForKey:KEY_MSGID];
    if( msgId == nil) {
        NSLog(@"message id is nil \r\n");
        return;
    }
    
    WEAK_SELF(weak_self);
    
    CDVInvokedUrlCommand* command = [weak_self.pendingMessageMap objectForKey:msgId];
    if( command == nil) {
        NSLog(@"call back id is nil \r\n");
        return ;
    }
    ...
    [self.pendingMessageMap removeObjectForKey:msgId];
...
//        CDVPluginResult* pluginResult = nil;
        if (error == nil) {
            ...
            [weak_self.pendingMessageMap setValue:command forKey:message.msgId];
            //[conversation sendTextMessage:text];
...
-(void)commonPushRespone:(NSString *)functionName
     ...
     NSLog(@"JMessagePlugin Log: functionName(%@) jsonString(%@) succeeded",functionName,jsonString);
...
-(void)networkDidReceiveNotification:(NSNotification *)notification{
    NSString * notificationInfo = [notification.object toJsonString];
