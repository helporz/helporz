...
public class JMessagePlugin extends CordovaPlugin {
    ...
    protected void pluginInitialize() {
        ...
        IMReceiver.getInstance().init(mCordovaActivity,cordova,webView);
...
//        JMessageClient.registerEventReceiver(this);
    ...
    public void sendSingleTextMessage(JSONArray data, final CallbackContext callback) {
        ...
        try {
            ...
            String json = mGson.toJson(msg);
            Log.i(TAG, "sendSingleTextMessage message \n" + json);
    ...
    static public class IMReceiver {
        private Activity mCordovaActivity;
        ...
        private static IMReceiver instance =  new IMReceiver();
        public class PendingMessage {
            String eventName;
            String jsonString;
        }

        private List<PendingMessage> pendingMessageList = new LinkedList<PendingMessage>();
        public static IMReceiver getInstance() {
            return instance;
        }
        ...
        public void init(Activity actvt,CordovaInterface cordova,CordovaWebView webView) {
            mCordovaActivity = actvt;
            mCordova = cordova;
            mWebView = webView;
            JMessageClient.init(mCordova.getActivity().getApplicationContext());
            JMessageClient.registerEventReceiver(this);


            for( PendingMessage pm : pendingMessageList) {
                fireEvent(pm.eventName,pm.jsonString);
            }
        ...
        public void onEvent(MessageEvent event) {
            ...
            String jsonStr = mGson.toJson(msg);
            fireEventWrap("onReceiveMessage", jsonStr);
            ...
            try {
                switch (msg.getContentType()) {
                    case text:  // 处理文字消息。
                        ...
                        JSONObject textJson = new JSONObject();
                        textJson.put("text", text);
                        fireEventWrap("onReceiveTextMessage", textJson.toString());
                        break;
                    case image:
                        ImageContent imageContent = (ImageContent) msg.getContent();
                        String imagePath = imageContent.getLocalPath();
                        ...
                        imageJson.put("imagePath", imagePath);
                        imageJson.put("thumbnailPath", thumbnailPath);
                        fireEventWrap("onReceiveImageMessage", imageJson.toString());
                        break;
                    case voice:
                        VoiceContent voiceContent = (VoiceContent) msg.getContent();
                        String voicePath = voiceContent.getLocalPath();
                        ...
                        voiceJson.put("voicePath", voicePath);
                        voiceJson.put("duration", duration);
                        fireEventWrap("onReceiveVoiceMessage", voiceJson.toString());
                        break;
                    case custom:
                        CustomContent customContent = (CustomContent) msg.getContent();
                        // 获取自定义的值。
                        ...
                        fireEventWrap("onReceiveCustomMessage", customJson.toString());
                        break;
                    case eventNotification:
                        EventNotificationContent content = (EventNotificationContent)
                                msg.getContent();
                        switch (content.getEventNotificationType()) {
                            case group_member_added:    // 群成员加群事件。
                                fireEventWrap("onGroupMemberAdded", null);
                                break;
                            case group_member_removed:
                                // 群成员被踢事件（只有被踢的用户能收到此事件）。
                                fireEventWrap("onGroupMemberRemoved", null);
                                break;
                            case group_member_exit:
                                fireEventWrap("onGroupMemberExit", null);
                                break;
                            default:
                        }
                        break;
        ...
        public void onEvent(LoginStateChangeEvent event) {
            ...
            switch (reason) {
                case user_password_change:
                    fireEventWrap("onUserPasswordChanged", null);
                    break;
                case user_logout:
                    fireEventWrap("onUserLogout", null);
                    break;
                case user_deleted:
                    fireEventWrap("onUserDeleted", null);
                    break;
                default:
            }
        }
        ...
        public void onEvent(NotificationClickEvent event) {
            Message msg = event.getMessage();
            String json = mGson.toJson(msg);
            fireEventWrap("onOpenMessage", json);
            Log.i(TAG, " onOpenMessage:" + json);
            Intent launch = mCordova.getActivity().getApplicationContext()
                    .getPackageManager().getLaunchIntentForPackage(
                            mCordova.getActivity().getApplicationContext().getPackageName());
            launch.addCategory(Intent.CATEGORY_LAUNCHER);
            launch.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_SINGLE_TOP);
            mCordova.getActivity().getApplicationContext().startActivity(launch);
        }
        ...
        private void fireEvent(String eventName, String jsonStr) {
            ...
            mCordovaActivity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    mWebView.loadUrl("javascript:" + finalJs);
                }
            });
        }

        @TargetApi(Build.VERSION_CODES.JELLY_BEAN_MR1)
        private void fireEventWrap(String eventName, String jsonString) {
            if( mCordovaActivity.isDestroyed() || mCordovaActivity.isFinishing()) {
                PendingMessage pm = new PendingMessage();
                pm.eventName = eventName;
                pm.jsonString = jsonString;
                pendingMessageList.add(pm);
            }
            else {
                fireEvent(eventName, jsonString);
            }
        }
